...
filename = "Ex2TLDP(" + currentDT.strftime("%H-%M-%S %Y-%m-%d") + ").txt"
                ...
                if key in observation[i]:
                    ...
                    if rand < confidence:
                        ...
                        if ob3 == {}:
                            ...
                            selected_bot = random.choice(NEIGHBOR_NUM)
                            communication += 1
                            # communication[i+1] += 1
                            ...
                            if key in observation[selected_bot]:
                                ...
                                if ob2 != {}:
                                    ...
                                    distribution[i][key] = distribution[selected_bot][ob2] 
                            ...
                            distribution[i][key] = distribution[i][ob3] 
                            # generate an action for each bot according to the distribution
                            # print("distribution = ", distribution[i][key])
                            random_ = np.random.rand()
                            for m in range(len(self.actions)):
